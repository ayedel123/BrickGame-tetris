...
int checkCollision2(GameManager *gameManager, Brick *brick, int direction) {
  int result = 0;
...
void moveBrickInField(int **field, Brick *brick) {
  ...
  for (int i = 0; i < 4; i++) {
    field[brick->y + brick->cords[i][1]][brick->x + brick->cords[i][0]] = 1;
  }
}

int moveBrick2(GameManager *gameManager, Brick *oldBrick, int direction,
               int angle) {
  ...
  int result = checkCollision2(gameManager, &newBrick, direction);
  if (result == COL_STATE_NO_COL) {
    *oldBrick = newBrick;
  }
...
int handleAction(GameManager *gm, int direction, int angle) {
  ...
  result =
      (result == COL_STATE_CRIT)
          ? result
          : moveBrick2(gm, &gm->bricks[gm->current_brick], direction, angle);
...
int moveBrick(GameManager *gameManager, int direction) {
  ...
  // moveBrick2(gameManager, &gameManager->bricks[gameManager->current_brick],
  //            direction);

  // if (checkCollision2(gameManager,
  //                     &gameManager->bricks[gameManager->current_brick]) != 0)
  //                     {
  //   moveBrick2(gameManager, &gameManager->bricks[gameManager->current_brick],
  //              -direction);
  //   // move_cursor(gameManager, direction);
